...
if (!rvcheck::check_github("guangchuangyu/nCov2019")$up_to_date){
...
```{r}
valueBox(x$chinaAdd$confirm,color = colors[["confirm"]],icon="fa-angle-double-up")
```

### 较昨日上升

```{r}
valueBox(x$chinaAdd$suspect,color = colors["suspect"],icon="fa-angle-double-up")
```
...
```{r}
valueBox(x$chinaAdd$heal,color = colors[["heal"]],icon="fa-angle-double-up")
```

### 较昨日上升

```{r}
valueBox(x$chinaAdd$dead,color = colors[["dead"]],icon="fa-angle-double-up")
```
...
```{r}
df <- summary(x2,province="湖北") %>% 
  mutate(date=as.Date(time,format="%m.%d")) %>%
  select(date,cum_confirm,cum_dead,cum_heal) %>%
  rename("confirm"=cum_confirm,
         "dead"=cum_dead,
         "heal"=cum_heal) %>%
  mutate_at(c("confirm","dead","heal"),as.numeric) %>%
  pivot_longer(cols = -date)
...
```{r}
df <- summary(x2,by="today",province="湖北") %>% 
  mutate(date=as.Date(time,format="%m.%d")) %>%
  select(date,confirm,dead,heal) %>%
  mutate_at(c("confirm","dead","heal"),as.numeric) %>%
  pivot_longer(cols = -date)
